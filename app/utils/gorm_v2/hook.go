	...
	if reflect.TypeOf(gormDB.Statement.Dest).Kind() != reflect.Ptr {
		...
		if destValueOf.Type().Kind() == reflect.Slice || destValueOf.Type().Kind() == reflect.Array {
			...
			for i := 0; i < inLen; i++ {
				...
				if row.Type().Kind() == reflect.Struct {
					if b, column := structHasSpecialField("CreatedAt", row); b {
						destValueOf.Index(i).FieldByName(column).Set(reflect.ValueOf(variable.NowTimeSH().Format(variable.DateFormat)))
					}
					...
					if b, column := structHasSpecialField("created_at", row); b {
						row.SetMapIndex(reflect.ValueOf(column), reflect.ValueOf(variable.NowTimeSH().Format(variable.DateFormat)))
					}
			...
			if b, column := structHasSpecialField("CreatedAt", gormDB.Statement.Dest); b {
				gormDB.Statement.SetColumn(column, variable.NowTimeSH().Format(variable.DateFormat))
			}
			...
			if b, column := structHasSpecialField("created_at", gormDB.Statement.Dest); b {
				destValueOf.SetMapIndex(reflect.ValueOf(column), reflect.ValueOf(variable.NowTimeSH().Format(variable.DateFormat)))
			}
