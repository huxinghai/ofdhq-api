...
const (
	...
	RobotLiveTokenExpireErrCode       int64  = 10909
)

var TwoStepVerifiErr = errors.New("二次验证失败！")
var StrategyBaseDuplicationErr = errors.New("SID与SSID已经添加策略不能重复添加")
var UserBalanceErr = errors.New("用户余额不足")
var BuyStrategyDuplicate = errors.New("用户已经存在不能重复购买")
var BuyUnPublishedErr = errors.New("策略已经停止购买")
var ErrDuplicateRequest = errors.New("该重复请求")
var RobotLiveTokenExpireErr = errors.New("实盘分享链接已过期")

type ErrBusiness struct {
	Message string //前端提示错误信息
	Err     error  //日志打印
}

func NewErrBusiness(msg string, err error) *ErrBusiness {
	return &ErrBusiness{
		Message: msg,
		Err:     err,
	}
