...
// 使用工厂创建一个 JWT 结构体
func CreateAdminWT(signKey string) *AdminJwtSign {
	if len(signKey) <= 0 {
		signKey = "ofdhq-api-admin"
	}
	return &AdminJwtSign{
		[]byte(signKey),
	}
...
// 定义一个 JWT验签 结构体
type AdminJwtSign struct {
	SigningKey []byte
}

// CreateToken 生成一个admin token
func (j *AdminJwtSign) CreateToken(claims AdminCustomClaims) (string, error) {
...
// 解析Token
func (j *AdminJwtSign) ParseToken(tokenString string) (*AdminCustomClaims, error) {
...
labelHere:
	if claims, ok := token.Claims.(*AdminCustomClaims); ok && token.Valid {
		return claims, nil
	} else {
		return nil, errors.New(my_errors.ErrorsTokenInvalid)
	}
...
// 更新token
func (j *AdminJwtSign) RefreshToken(tokenString string, extraAddSeconds int64) (string, error) {
